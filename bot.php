...
//Текст ответа 
$texed = array('Привет, меня сейчас нет на месте, но я скоро буду. Если это что-то очень срочное, напиши тут — https://hetop.xyz/message/ и я с тобой свяжусь.'); 
...
echo curl('https://api.telegram.org/bot'.$tg_token.'/sendMessage?chat_id=359787060&text=Вам сообщение от '.$sendName.$sendMessageText.$sendPhoto.$sendVideo.$sendAudio.'&reply_markup='.$replyMarkup);
} 
function curl($url){ 
$ch = curl_init( $url ); 
